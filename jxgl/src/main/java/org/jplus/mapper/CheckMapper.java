...
public interface CheckMapper {
    ...
    //<!--院系  更新审核状态为1 -->
    @Update("update `tjzt` set bksshzt = 2, yxbksshrxm =#{username} , yxbksshsj = #{date}  where gh=#{gh}")
    public void setShztByDepartments(@Param("gh") String gh,@Param("username") String userName,@Param("date") Timestamp date);

    //<!--教务处 更新审核状态为2 -->
    @Update("update `tjzt` set bksshzt = 3, jwcshrxm =#{username} , jwcshsj = #{date}  where gh=#{gh}")
    ...
    @Update("update `tjzt` set yjsshzt = 2, yxyjsshrxm =#{uname} , yxyjsshsj = #{date}  where gh=#{gh}")
    public void setShztByDepartmentsY(String gh, String uname, Timestamp date);

    @Update("update `tjzt` set yjsshzt = 3, yjscshrxm =#{username} , yjscshsj = #{date}  where gh=#{gh}")
    public void setShztByOfficeY(@Param("gh") String gh,@Param("username") String userName,@Param("date") Timestamp date);
}
