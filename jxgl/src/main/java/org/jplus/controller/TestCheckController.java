...
public class TestCheckController {
    ...
    public String getQualityEngineeringInfo(@ModelAttribute(value = "gh") String gh,
        ...
        Integer tjzt = tjztService.getTjzt(gh).getBkstjzt();
        if(tjzt == 0){
            model.addAttribute("hidden", 1);
        }else {
            model.addAttribute("hidden", 0);
        }
        // 添加当前操作用户的信息
...
// **************************************具体信息***********************************************
        ...
        model.addAttribute("bksktjx", bksktjxService.getBksktjxInfo(gh, 2020));

        // 2 实践教学工作量         practiceteh
        Optional.ofNullable(bkssjjxService.selectBkssjjx(gh, xqnd)).ifPresent(sjjx -> model.addAttribute
                ("bkssjjx", getSjjxWork(sjjx)));
        model.addAttribute("bkssjjxInfo", checkService.getBKSSJJXInfo(gh));
        // 3 质量工程工作量        qualityengineering
        model.addAttribute("zlgc", zlgcService.getZlgcInfo(gh, xqnd));
        ...
        //  编写规划教材、教材获奖工作量
        //  教学改革工作量
        model.addAttribute("Jxcgs", jxcgService.getJxcgInfo(gh, xqnd));
        model.addAttribute("Jxcgdjs", jxcgService.getJxcgdjInfo());

        model.addAttribute("Zxxms", jxcgService.getZxxm(gh, xqnd));
        model.addAttribute("Zxxmdjs", jxcgService.getZxxmdjInfo());

        model.addAttribute("Hxxms", jxcgService.getHxxm(gh, xqnd));

        model.addAttribute("Jcs", jxcgService.getJc(gh, xqnd));
...
// **************************************具体信息***********************************************
    ...
    @NeedLogin
    @PostMapping("/getOfficedetailsByYear")
    public String getOfficedetailsByYear(@ModelAttribute(value = "xqnd") Integer integer){
        this.xqnd = xqnd;
        return "redirect:officedetails";
    }
/*

    @NeedLogin
    @GetMapping("/officedetailsYY")
    public String getQualityEngineeringInfoY(@ModelAttribute(value = "gh") String gh,
                                            @ModelAttribute(value = "userName") String userName,
                                            Model model, Users users) {
        // 添加当前操作用户的信息
        model.addAttribute("reviewer",users.getActor());
...
*/
    ...
    public String passCheck(@ModelAttribute(value = "gh") String gh, Users users, RedirectAttributes attributes) {
        ...
        // 如果是本科生院系负责人
        if (users.getActor() == 2){
            if (tjztService.getTjzt(gh).getBkstjzt() != 0) {
        ...
        if (users.getActor() == 4){
            if (tjztService.getTjzt(gh).getYjstjzt() != 0) {
                // tjzt 将审核状态设置为审核2 (院系审核) ,姓名，审核时间写进去
                checkService.setShztByDepartmentsY(gh,users.getUname(),date);
                // 将院系审核人的名字写到提交表tjb
                checkService.setShrgh(gh,users.getGh());
            }
        }
        ...
        if (users.getActor() == 3){
            if (tjztService.getTjzt(gh).getBkstjzt() != 0) {
                // tjzt  将审核状态设置为审核3 ( 教务处审核)姓名，审核时间写进去
        ...
        if (users.getActor() == 5){
            if (tjztService.getTjzt(gh).getYjstjzt() != 0) {
                // tjzt  将审核状态设置为审核3 ( 教务处审核)姓名，审核时间写进去
                checkService.setShztByOfficeY(gh,users.getUname(),date);
            }
        }
        ...
        if(users.getActor()==5){
            attributes.addAttribute("gh", gh);
            attributes.addAttribute("tempYxbm",checkService.getLxbmByGh(gh));
            return  "redirect:/departmentalaudit3";
        }
        else {
            return null;
        }
